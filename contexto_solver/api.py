...
class DistanceScoreMapper:
    ...
    def __init__(self, max_observed_distance: float = 70000.0, fit_min_samples: int = 40):
    ...
    def add_observation(self, distance: float, mapped_score: float) -> None:
        ...
        n = len(self._dist_buf)
        # Fit initially at 40 samples, then re-fit every 100 to incorporate new data.
        if n >= self.fit_min_samples and (self._iso is None or n % 100 == 0):
...
def make_api_feedback_fn(api: "ContextoAPI", bad_words: Set[str]):
    ...
    def fn(word: str) -> Tuple[float, dict]:
        ...
        # Mark bad words: explicit 404 only (lemma != word is valid for inflected forms)
        if raw.get("404"):
            bad_words.add(word)
        ...
        if isinstance(dist, (int, float)):
            try:
                ...
                dist_hit = raw_distance <= 1.0
            except Exception:
                pass

        state["raw_distance"] = raw_distance
